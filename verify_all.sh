...
for t in control/test.mjs control/test_graph.mjs control/test_hints.mjs \
         ...
         spatial/test.mjs auth/test.mjs venue/test.mjs evals/test.mjs bundles/test.mjs web/test_home.mjs web/test_progress.mjs web/test_ladder.mjs web/test_3d.mjs web/test_rtl.mjs; do
  out=$(node "$t" 2>&1); rc=$?
  n=$(printf '%s\n' "$out" | grep -c '^  ok ')
  total=$((total + n))
  want=$(readme_says "$t")
