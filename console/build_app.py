...
def bundle_control():
    ...
    for f in SOURCES:
        ...
        src = re.sub(r'^export\s+(async function|const|function|class|let)\s', r'\1 ', src, flags=re.M)
...
/* The pack stores no lesson rows. pack/registry.js - inlined above, the
   consumer library the Academy itself runs - generates every lesson of a
   hall by rule, and the console generates its served hall the same way:
   11,000 rows at boot, never stored, never restated. An earlier version
   hydrated stored per-union shards (the 33-hall pack's shape) that the
   slice had stopped carrying, and threw on its first line. The slice's one
   Python-generated sample row per hall is held against this mirror below,
   so the two copies of the rule cannot disagree without the page saying so. */
if (SHAPE.levels !== SLICE.shape.levels || SHAPE.slots !== SLICE.shape.slots)
  throw new Error('registry.js SHAPE disagrees with the slice shape');
if (JSON.stringify(BANDS) !== JSON.stringify(SLICE.variants.bands)
    || JSON.stringify(MODALITIES) !== JSON.stringify(SLICE.variants.modalities))
  throw new Error('registry.js bands/modalities disagree with variants.json');
for (const s of SLICE.lessons) {
  const h = SLICE.unions.find(u => u.slug === s.union);
  const mine = lessonOf(h, Number(s.lesson_id.slice(6, 9)), Number(s.lesson_id.slice(11, 14)));
  for (const k of ['lesson_id', 'skill_id', 'form', 'base_difficulty', 'state', 'level_test', 'final_exam'])
    if (mine[k] !== s[k]) throw new Error(`lesson rule drift on ${s.lesson_id}.${k}: ${mine[k]} vs ${s[k]}`);
}
// a placeholder title composed from the generated row - no lesson prose is
// authored yet, and the page says so
const titleOf = (l) => l.form[0].toUpperCase() + l.form.slice(1)
  + ' \u2014 ' + shortSkill(l.skill_id) + ' (' + l.track + ' L' + (l.level + 1) + ')';
const LESSONS = [];
for (let level = 0; level < SHAPE.levels; level++)
  for (let slot = 0; slot < SHAPE.slots; slot++) {
    const l = lessonOf(unionRow, level, slot);
    LESSONS.push({ ...l, title: titleOf(l) });
  }
const BY_SKILL = new Map();
for (const l of LESSONS) {
  if (l.state !== 'live') continue;                 // only live modules are servable
  if (!BY_SKILL.has(l.skill_id)) BY_SKILL.set(l.skill_id, []);
  BY_SKILL.get(l.skill_id).push(l);
}
// BANDS and MODALITIES are registry.js's, held equal to variants.json above;
// the offsets and ceilings are read from the registry slice itself
const OFFSET = SLICE.variants.band_offset;
...
# the prose counts are the slice's own, never typed: halls and skills as the
# slice carries them, lessons as the pack's ledger states them per hall
_slice = json.loads(slice_json)
_served = next(u for u in _slice['unions'] if u['slug'] == 'welders')
