...
// Visko Orbis Stable has a real audio track and real audio commands
// (see `AudioPanel.tsx`) - but both `set_audio_enabled` and
// `set_audio_prompt` are out of scope for THIS panel specifically, not
// because this model lacks them like Helios does. `set_audio_enabled`
// is idle-only (armed at `start`, this panel only ever touches an
// already-running session); `set_audio_prompt` is measured to make the
// audio worse, which is why `AudioPanel.tsx` leaves the box out too.
// See the header note in `../lib/flow.ts` for the full reasoning. So
// this panel, like the Helios one, only ever calls `setPrompt`.

// Optional, student-controlled pacing panel. Once a session is running
// (started via <PromptComposer /> or <ImageStarter />), this lets
// whoever is watching - the student themselves, or a teacher sitting
// with them - log a voluntary check-in and real lesson-performance
// numbers, and asks the backdrop to adapt: a calmer, simpler scene
// under load, a touch more motion when the pace is clearly too easy.
// It changes the ambient backdrop only - never the union-module prompt
// itself, never anything framed as a claim about the student.
//
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-3">
      ...
      <p className="mt-1 text-[11px] leading-snug text-zinc-500">
        ...
        never the module content, never a claim about you, never audio
        (see Audio above - it only arms at the next start).
      </p>
