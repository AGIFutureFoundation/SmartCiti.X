// Flow-state pacing logic for the SmartCiti.X Orbis runner - Visko Orbis
// Stable edition.
//
// Ported from the sibling Helios runner's `app/lib/flow.ts`, adapted to
// this app's REAL SDK: `useViskoOrbisStable()` / `sendSetPrompt` (see
// `../lib/visko.ts`), not `useHelios()`.
//
// A NOTE ON AUDIO, since this is the one real difference from Helios
// that a naive port would get wrong: unlike Helios, Visko Orbis Stable
// genuinely has a `main_audio` track and real `set_audio_prompt` /
// `set_audio_enabled` commands (see `../components/AudioPanel.tsx`).
// This module still makes no audio decision, for two separate reasons,
// not one:
//   1. `set_audio_enabled` is an idle-only knob - the SDK reads it once
//      when `start` fires and it survives `reset`, so it cannot affect
//      an ALREADY-RUNNING session, which is the only kind of session
//      this panel ever touches (see `isVideoDisabled` and the render
//      guard in `FlowPanel.tsx`).
//   2. `set_audio_prompt` is measured to make the generated audio WORSE
//      than leaving it unset (audio generated from the picture alone
//      scores higher) - `AudioPanel.tsx` already leaves this command
//      out of the UI entirely and says why; this module holds to the
//      same measured guidance rather than reintroducing the box here.
// So the live lever this file offers is exactly the one lever both
// models actually share while a session is running: `setPrompt` on the
// ambient backdrop. Restoring "audio fields" to `FlowDecision` just to
// mirror this model's fuller SDK surface would describe a control this
// panel cannot honestly exercise.
//
// The stateless decision logic lives here, framework-independent and
// unit-testable; the stateful piece - the React component that actually
// calls the hook - is `app/components/FlowPanel.tsx`.
//
 ...
 * this function is never used to build one of those). Carries no audio
 * direction - see the module note above for why.
 */
