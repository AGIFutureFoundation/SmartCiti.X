...
ten walkable worlds exist. Six are BUILT: real campuses, each with the
...
the subset of that stack a hub actually needs (see below). Four are
CANDIDATES: real US metros with real trade profiles, proposed as the
next four, and nothing more than proposed.

TWO PROVENANCE TIERS, NOT ONE. Three of the six built campuses'
...
Chicago and Seattle, the fourth, fifth and sixth, and the four remaining
candidates' city coordinates are AUTHORED: widely-published public
geography (the kind a city's own Wikipedia infobox states), typed here
from general knowledge rather than copied from any file this build can
check itself against. That is a materially weaker claim, and it is
labelled as one rather than dressed up as RECORDED. Promoting a
candidate to built means doing the checklist below in full, not just
relabelling an entry - Houston, Chicago and Seattle all did it as a HUB
campus rather than a district campus (see HUB_DESIGN below); a future
candidate could still go either way.
...
Houston, Chicago or Seattle. Instead each hosts no home district of its
own (districts: []) and the chapters mechanic every campus already had -
a regional chapter, for every hall, at every campus that is not its
home - simply gained a fourth, fifth and sixth destination: Houston,
Chicago and Seattle are each now a regional-chapter seat for all 111
halls, real walkable points on the network map, and duplicate no hall's
content anywhere. None draws a district ring nor carries a
...
HONESTY = {
    'target_not_claim': 'ten walkable worlds is a target this registry '
                        ...
                        'exist. Six are built; four are candidates.',
    'provenance_tiers': 'three of the six built campuses\' coordinates '
                        'are RECORDED - copied from a cited file and '
                        'cross-checked against it on every build. '
                        'Houston, Chicago and Seattle, the fourth, fifth '
                        'and sixth, and the four remaining candidates\' '
    ...
    'hub_vs_district': 'three built campuses are district campuses: a '
                       ...
                       'Chicago and Seattle are hub campuses: no home '
                       'district, no district ring, no parcel-records '
                       'contract - and a real regional chapter seat for '
                       'all 111 halls each, the same mechanic every '
                       'campus already had, extended to a fourth, fifth '
                       'and sixth destination. Building one shape does '
                       'not oblige the next candidate to be the other.',
