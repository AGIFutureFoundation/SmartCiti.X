 ...
 * three campuses are BUILT on RECORDED, cross-checked coordinates: seven
 * are CANDIDATES on AUTHORED, not-cross-checked ones. The suite holds
 * both halves to their own bar - the built entries must still trace to
 * the geo registry's own cross-checked figures, and the candidates must
...
ok('every built entry\'s coordinate is the geo registry\'s own RECORDED/DERIVED figure',
...
ok('built entries carry a real hall count off the union registry',
  built.every(([k, b]) => b.halls === campuses[k].halls.length && b.halls > 0));
...
ok('the seven candidates are seven distinct real US metros',
