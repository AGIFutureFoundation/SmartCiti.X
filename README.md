...
| `roadmap/` | — | **The network roadmap**: the path from three built campuses to ten walkable worlds, tracked as a target rather than claimed as an achievement. The three built entries are the union registry's own campuses, RECORDED and cross-checked exactly as `geo/` already does. Seven candidates — real US metros with real trade profiles (Houston's energy corridor, Chicago's rail hub, Seattle's aerospace and port trades, Pittsburgh's steel heritage, Denver's mining and altitude work, Miami's hurricane-code envelope discipline, Detroit's heavy manufacturing) — carry **AUTHORED** coordinates: widely-published public geography, typed from general knowledge rather than copied from a citable file, and labelled as the materially weaker claim it is. A candidate names a proposed metro and a proposed district emphasis only — no hall, union or curriculum content is claimed to exist there, and the registry carries the actual checklist (source real coordinates, cross-check them, register the campus, build the 3D geometry, give it a world, verify and ship) a candidate has to clear to become built, the same way the first three were |
