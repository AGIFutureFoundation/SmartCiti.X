...
node geo/test.mjs                  # 32 — real coordinates + anchors (RECORDED, citing Locator.X, near the three flagship campuses; AUTHORED, admittedly not cross-checked, near the four hub campuses), recomputed distances, GeoJSON + the network map file
...
node roadmap/test.mjs              # 34 — the network roadmap: 7 built (3 district campuses + 4 regional-chapter hubs) + 3 candidates (AUTHORED) totalling exactly 10, placed by real bearing/distance on the 3D region board and at their own real coordinate on the network geomap
...
|---|---|---|
| `unions/` | — | **The union registry**, separated from the module pack: the 111-hall taxonomy, its eight districts and the seven-campus network — three district campuses (Treasure Island–San Francisco, Oakland, New Orleans) plus Houston, Chicago, Seattle and Pittsburgh, four regional-chapter **hub** campuses with no home district of their own — a builder with a source stamp, and a verifier that proves the module pack still agrees with the roster |
...
| `roadmap/` | — | **The network roadmap**: the path from seven built campuses to ten walkable worlds, tracked as a target rather than claimed as an achievement. The seven built entries are the union registry's own campuses; three are district campuses, RECORDED or DERIVED and cross-checked exactly as `geo/` already does, and four — Houston, Chicago, Seattle and Pittsburgh — are **hub** campuses: no home district, no district ring, and a regional-chapter seat for all 111 halls instead, on **AUTHORED** coordinates like a candidate's. Three candidates — real US metros with real trade profiles (Denver's mining and altitude work, Miami's hurricane-code envelope discipline, Detroit's heavy manufacturing) — carry the same AUTHORED coordinates: widely-published public geography, typed from general knowledge rather than copied from a citable file, and labelled as the materially weaker claim it is. A candidate names a proposed metro and a proposed district emphasis only — no hall, union or curriculum content is claimed to exist there, and the registry carries the actual checklist (source real coordinates, cross-check them, register the campus, build the 3D geometry, give it a world, verify and ship) a candidate has to clear to become built, the same way the built campuses were. Live on two more surfaces now, not just the flat dashboard: the 3D region board places each candidate at its real great-circle bearing and distance from the flagship campus as a dashed schematic ring — the labels doctrine's own "the dashed outline is the claim" — and the network geomap, a real WGS84 map, places each one at its own actual AUTHORED coordinate; both click through to a card with the same registry honesty text, never a walkable campus |
