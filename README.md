...
**768 checks, all passing from inside this bundle**, with no model credentials
required.
...
```bash
node unions/verify.mjs             # 26 — roster, districts, campuses, chapters, pack agreement
...
node geo/test.mjs                  # 32 — real coordinates + anchors (RECORDED, citing Locator.X, near the three flagship campuses; AUTHORED, admittedly not cross-checked, near the seven hub campuses), recomputed distances, GeoJSON + the network map file
node sims/test.mjs                 # 32 — 9 simulators bound to 41 real halls, each with a scripted reference operator (3 levels, guarantees asserted against the rubric, procedure ids the page's policy is written around) (crane-lift/rigging-signals/load-chart extend to every trade that actually rigs, signs or plans a pick — ironworkers, decking, tank-erectors, piling; weld-bead to marine-pipe and tank-erectors; scaffold-bay to the rest of the envelope district plus refractory; pressure-washer to laborers and hazmat alongside painters), rubrics, cockpit + scoring + regional scenarios
...
node training/test.mjs             # 42 — the robotic training-data recorder: episode shapes, the storage/export contract, the not-scored guarantee, TRACE's own gauge-sampling add-on, the Orbis cross-link, the `actor` field and the SCRIPTED tier (never AI-SYNTHESIZED), the balanced-paren pass-expression guard
...
node meta/test.mjs                 # 26 — the metaverse layer: standards, the Unity avatar review, the complete VRM skeleton and how it moves, import policy, honesty, now on the network dashboard too
...
| `restoration/` | — | **The Bay Restoration pack**: 10 real sites across two categories — nine real, independently-run San Francisco Bay habitat-restoration sites and programs (Heron's Head Park, Candlestick Point, the South Bay Salt Pond ecotone levee, Montezuma wetlands, American Canyon, the North Bay STRAW program, Alviso, an East Oakland youth program, and the bay-wide invasive-Spartina removal effort) — AUTHORED FROM PUBLIC RECORD (this build reaches no network host, so nothing here is fetched or cross-checked against sfbayrestore.org live), each linking to its own real source page and, where one exists, its own real workforce-development pathway (Eco-Apprentice, STRAW, Planting Justice's youth program) — none of them a SmartCiti.X program; plus one environmental-monitoring site, **Hunters Point Naval Shipyard**, a real, currently-litigated federal Superfund cleanup site in San Francisco's Bayview-Hunters Point neighborhood (Navy-led under CERCLA, EPA Region 9 and California DTSC oversight) — its facts are a September 2026 snapshot of public record (a settled $57M Tetra Tech data-fraud settlement, and Greenaction's still-open 2024 lawsuit against the Navy over cleanup adequacy, each independently cited, never conflated), and it points to the real, community-run Marie Harrison Bayview Air Monitoring Project (Greenaction / IVAN BVHP) rather than any invented SmartCiti.X program or workforce pathway. Every one of the 10 sites also names its real `trade_needs` — real union slugs from `unions/registry/unions.json` whose own registered focus text actually matches that site's field work (never forced onto a site it doesn't fit). Three field-skill tracks bridge the habitat-restoration field work to real skill_ids this bundle's own graph already ships (grounds, site-safety, laborers, surveyors, survey-drone) — no restoration-only skill is invented. Live on the network geomap (9 sites mapped at their own real coordinate, visually distinct from campuses, roadmap candidates and from each other by category), on the network dashboard (site / track count and its own honesty line), and in its own 🌊 3D-app panel, where every field-skill track clicks straight through to the hall that actually teaches it — and any hall that teaches one links straight back, badge and all, the same bidirectional pattern proven for Schools and Orbis. Eight of the nine mapped sites — the habitat-restoration ones — also stand as real, walkable markers in the Treasure Island / Oakland city layer, and each opens its own standalone ground-level scene — schematic terrain composed from that exact site's own real habitat/scale/org record, never a shared template, honestly captioned as schematic rather than surveyed — with three in-world field-skill-track beacons a learner can walk up to and click straight through to the real hall. Hunters Point is explicitly `walkable: False`: it is pinned and clickable on the map with its own flat facts-and-citations panel — the same flat-panel pattern the elevation/aerial-imagery lookups already use elsewhere in this pack — but it never renders as a walk-through ground scene, because an actively litigated federal cleanup site is not something this platform can responsibly present as a place to stroll |
