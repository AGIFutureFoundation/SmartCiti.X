...
| `restoration/` | — | **The Bay Restoration pack**: nine real, independently-run San Francisco Bay habitat-restoration sites and programs (Heron's Head Park, Candlestick Point, the South Bay Salt Pond ecotone levee, Montezuma wetlands, American Canyon, the North Bay STRAW program, Alviso, an East Oakland youth program, and the bay-wide invasive-Spartina removal effort) — AUTHORED FROM PUBLIC RECORD (this build reaches no network host, so nothing here is fetched or cross-checked against sfbayrestore.org live), each linking to its own real source page and, where one exists, its own real workforce-development pathway (Eco-Apprentice, STRAW, Planting Justice's youth program) — none of them a SmartCiti.X program. Three field-skill tracks bridge that real field work to real skill_ids this bundle's own graph already ships (grounds, site-safety, laborers, surveyors, survey-drone) — no restoration-only skill is invented. Live on the network geomap (eight sites mapped at their own real coordinate, visually distinct from campuses and roadmap candidates), on the network dashboard (site / track count and its own honesty line), and in its own 🌊 3D-app panel, where every field-skill track clicks straight through to the hall that actually teaches it — and any hall that teaches one links straight back, badge and all, the same bidirectional pattern proven for Schools and Orbis. Those eight mapped sites also stand as real, walkable markers in the Treasure Island / Oakland city layer, and each opens its own standalone ground-level scene — schematic terrain composed from that exact site's own real habitat/scale/org record, never a shared template, honestly captioned as schematic rather than surveyed — with three in-world field-skill-track beacons a learner can walk up to and click straight through to the real hall |
