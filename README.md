...
node geo/test.mjs                  # 32 — real coordinates + anchors (RECORDED, citing Locator.X, near the three flagship campuses; AUTHORED, admittedly not cross-checked, near the two hub campuses), recomputed distances, GeoJSON + the network map file
...
|---|---|---|
| `unions/` | — | **The union registry**, separated from the module pack: the 111-hall taxonomy, its eight districts and the five-campus network — three district campuses (Treasure Island–San Francisco, Oakland, New Orleans) plus Houston and Chicago, two regional-chapter **hub** campuses with no home district of their own — a builder with a source stamp, and a verifier that proves the module pack still agrees with the roster |
