...
**677 checks, all passing from inside this bundle**, with no model credentials
required.
...
node training/test.mjs             # 29 — the robotic training-data recorder: episode shapes, the storage/export contract, the not-scored guarantee, TRACE's own gauge-sampling add-on
node orbis/test.mjs                # 65 — the Orbis synthetic-training prompt contract + the two real video-model runners, now cross-linked from the panel itself: no key shipped, no network reached, AI-SYNTHESIZED labelled
