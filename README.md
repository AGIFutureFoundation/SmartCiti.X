...
node geo/test.mjs                  # 32 — real coordinates + anchors (RECORDED, citing Locator.X, near the three flagship campuses; AUTHORED, admittedly not cross-checked, near the three hub campuses), recomputed distances, GeoJSON + the network map file
...
node roadmap/test.mjs              # 34 — the network roadmap: 6 built (3 district campuses + 3 regional-chapter hubs) + 4 candidates (AUTHORED) totalling exactly 10, placed by real bearing/distance on the 3D region board and at their own real coordinate on the network geomap
...
|---|---|---|
| `unions/` | — | **The union registry**, separated from the module pack: the 111-hall taxonomy, its eight districts and the six-campus network — three district campuses (Treasure Island–San Francisco, Oakland, New Orleans) plus Houston, Chicago and Seattle, three regional-chapter **hub** campuses with no home district of their own — a builder with a source stamp, and a verifier that proves the module pack still agrees with the roster |
