...
**388 checks, all passing from inside this bundle**, with no model credentials
required.
...
node avatars/test.mjs              # 15 — locker depth, the crew roster, marks policy, the cosmetic-only guarantee
...
| `avatars/` | — | **The avatar pack**: a humanoid locker — 15 standard sections at 15–20 options each (build, skin, hair style + colour, eyes, facial hair, headwear from hard hats to ball caps, tops, vest, trousers, footwear, tool belts — 239 options) plus the **crew section seating all 111 halls**, each stamping its three-letter-code mark in the district hue on vest, shirt and headwear (the Academy's own insignia — no real union's logo is drawn), and 8 emotes with emoji and procedural moves on a capsule-built humanoid with real facial features; **cosmetic only** — every option free and unlocked, none read by any grader, and the suite asserts it |
