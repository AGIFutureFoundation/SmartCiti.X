...
ok('the AUTHORED points (Houston, Chicago, Seattle - all hub campuses) admit they are not cross-checked - the same bar a roadmap candidate is held to',
  ['houston', 'chicago', 'seattle'].every((k) =>
...
ok('the GeoJSON is a FeatureCollection with one campus Point per campus',
  gj.type === 'FeatureCollection' && campusFeats.length === 6
...
const AUTHORED_CKS = ['houston', 'chicago', 'seattle'];
ok('every campus carries anchors — RECORDED citing their Locator.X table for '
  ...
  + 'AUTHORED for the three hub campuses that have no such sibling table',
  Object.keys(reg.anchors).length === 6
...
ok('the AUTHORED anchors (Houston, Chicago, Seattle) admit plainly they are not RECORDED '
...
ok('the GeoJSON carries the anchors as tagged features, [lng, lat]',
  gj.features.filter((f) => f.properties.kind === 'anchor').length === 31
...
ok('every AUTHORED city frame (the three hub campuses) admits it is not cross-checked, and still actually frames its campus',
...
ok('the network GeoJSON carries the whole registry: 37 points (6 campuses + 31 '
  + 'anchors), 15 route lines (every campus pair), 5 frames (2 RECORDED, 3 AUTHORED)',
  net.features.filter((f) => f.geometry.type === 'Point').length === 37
  && net.features.filter((f) => f.geometry.type === 'LineString').length === 15
  && net.features.filter((f) => f.geometry.type === 'Polygon').length === 5
